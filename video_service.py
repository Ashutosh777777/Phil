...
def generate_base_frame(speaker, author_a, author_b, bg_img, font_bold):
    """Generate the base frame (highlight + names only, no speech bubbles)."""
    img = apply_speaker_highlight(bg_img, speaker, author_a)
    overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    # Only draw name labels, no speech bubbles
    draw_name_labels(draw, font_bold, speaker, author_a, author_b)
    img = img.convert("RGBA")
    img = Image.alpha_composite(img, overlay)
    return img.convert("RGB")
...
def compose_video(
    ...
    for i, line in enumerate(script):
        ...
        # Generate base frame (highlight + names only, no speech bubbles)
        base_img = generate_base_frame(speaker, author_a, author_b, bg, font_bold)
